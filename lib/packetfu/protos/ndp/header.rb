...
module PacketFu
  ...
  class NDPHeader < Struct.new(:ndp_reserved, :ndp_tgt, :ndp_opt_type,
    ...
    def read(str)
      ...
      self[:ndp_reserved].read(str[0,4])
      self[:ndp_tgt].read(str[4,16])
      self[:ndp_opt_type].read(str[20,1])
      self[:ndp_opt_len].read(str[21,1])
      self[:ndp_lla].read(str[22,str.size])
      self
    end
